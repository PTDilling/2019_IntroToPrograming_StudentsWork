...
void setup() {
  ...
  myFlower3 = new Flower(_r3, _petals, _x+50, _y-offset, _pc, 7, 50);
}
...
void draw() {
  ...
  float d=dist(myFlower1._x, myFlower1._y, myFlower2._x, myFlower2._y);
  if (d<myFlower1._1 + myFlower2._r2) {
    _pc=#2fA2fa
  }
}
...
class Flower {
  ...
  Flower(float temp_r, int temp_n_petals, float temp_x, float temp_y, int temp_petalColor, float temp_speed, int temp_alpha) {
    r=temp_r;
    n_petals = temp_n_petals;
    x=temp_x;
    y=temp_y;
    petalColor=temp_petalColor;
    flowerAlpha=temp_alpha;
    speed = temp_speed;
  }
